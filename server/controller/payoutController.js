...
const createPayout = async (req, res) => {
  try {
    ...
    const isExistingWallet = await prisma.wallet.findFirst({
      where: { userId, accountNo: accountNumber },
    });

    if (isExistingWallet) {
      return res.status(400).json({
        ok: false,
        message: "Wallet already exists for this user and account number",
      });
    }
