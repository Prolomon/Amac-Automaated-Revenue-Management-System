...
import { API_URL, AUTH_MEMBER_REFRESH_TOKEN, AUTH_MEMBER_TOKEN } from "./api-config";
...
export function isTokenExpired(token: string | null | undefined, bufferSeconds = 60): boolean {
  ...
  if (!decoded || typeof decoded.exp !== "number") {
    // If not a standard JWT or missing exp, assume expired if empty
    return false;
  }
...
/**
 * Get the current access token (from memory or storage).
 */
export async function getAccessToken(): Promise<string | null> {
  if (memoryAccessToken) return memoryAccessToken;
  try {
    const stored = await AsyncStorage.getItem(AUTH_MEMBER_TOKEN);
    if (stored) {
...
export async function getRefreshToken(): Promise<string | null> {
  ...
  try {
    const stored = await AsyncStorage.getItem(AUTH_MEMBER_REFRESH_TOKEN);
    if (stored) {
...
export async function refreshAccessToken(): Promise<string | null> {
  ...
  activeRefreshPromise = (async () => {
    try {
      ...
      // Call server token refresh endpoint
      ...
      if (!response.ok || !data.ok) {
        // If refresh token is expired or invalid, trigger failure
        if (
          response.status === 401 ||
          response.status === 403 ||
 ...
 * If the current token is missing or expired, it automatically refreshes it first.
 */
