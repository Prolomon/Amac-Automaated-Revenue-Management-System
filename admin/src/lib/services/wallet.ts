...
export async function resolveBankAccount(
  accountNumber: string,
  bankCode: string,
): Promise<{ data?:{accountName: string; accountNumber: string}, message?: string, ok: boolean }> {
...
export async function updateWallet(
  ...
  bankName: string,
  role: "MEMBER" | "ADMIN" | "AGENT" | "COMPANY" | "STAFF",
): Promise<{ ok: boolean; wallet?: Wallet; message?: string }> {
  const response = await fetch(`${API_URL}/wallet/${id}/${role}`, {
    method: "PUT",
    headers: { ...buildHeaders(true) },
    body: JSON.stringify({ accountNo, accountName, bankCode, bankName }),
  });
  const data = await response.json();
  return data;
}
