...
export async function getCompanies(uid: string, page: number, limit: number): Promise<{ ok: boolean; data?: Company[]; message?: string, meta: { total: number; limit: number; page: number; totalPages: number } }> {
  if (!uid) {
    throw new Error("No user ID found");
  }
  const response = await fetch(`${API_URL}/company/center/${uid}?page=${page}&limit=${limit}`, {
