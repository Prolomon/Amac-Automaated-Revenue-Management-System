...
export async function getPayment(
  id: string,
): Promise<{ ok: boolean; payment?: Payment; message?: string }> {
...
export async function getAllPayments(
  ...
  const response = await fetch(
    `${API_URL}/payment?page=${page}&limit=${limit}${search ? `&search=${search}` : ""}`,
    {
      headers: { ...buildHeaders() },
    },
  );
...
export async function getPaymentsByPartner(
  partnerId: string,
): Promise<{ ok: boolean; payments?: Payment[]; message?: string }> {
...
export async function getRecord(
  ...
  const response = await fetch(
    `${API_URL}/payment-transaction/reference/${id}`,
    {
      headers: { ...buildHeaders() },
    },
  );
