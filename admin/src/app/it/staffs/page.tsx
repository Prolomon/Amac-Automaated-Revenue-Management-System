...
export default function StaffsPage() {
  ...
  const handleDownload = () => {
    try {
      ...
      addToast("error", "Failed to export staff data");
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <Link
              ...
              href={readOnly ? "/it/staffs" : "/it/staffs/add"}
            >
              <Plus size={18} />
          ...
          <div className="overflow-x-auto block">
            <table className="min-w-max w-full text-left">
              <thead className="border-b border-slate-200">
                <tr>
                  <th className="px-3 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 md:px-4 md:text-sm">
                    Staff ID
                  </th>
                  <th className="px-3 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 md:px-4 md:text-sm">
                    Full Name
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  filteredStaffs.map((staff) => (
                    ...
                    >
                      <td className="px-3 py-4 font-mono text-xs md:px-4 md:text-sm">
                        <Link
                          href={`/it/staffs/${staff.uid}`}
                          className="rounded-lg text-xs font-medium text-slate-600 transition-colors hover:text-emerald-600 md:text-sm"
                        >
                          {staff.uid}
                        </Link>
                      </td>
                      <td className="px-3 py-4 text-xs font-medium text-slate-900 md:px-4 md:text-sm truncate">
                        {truncate(staff.fullname, 16)}
                      </td>
                      <td className="px-3 py-4 text-xs text-slate-600 md:px-4 md:text-sm">
                        {truncate(staff.email, 20) || "—"}
                      </td>
                      <td className="px-3 py-4 text-xs text-slate-600 md:px-4 md:text-sm">
                        {truncate(staff.phone, 15) || "—"}
                      </td>
                      <td className="px-3 py-4 text-xs text-slate-600 md:px-4 md:text-sm capitalize">
                        {staff.gender || "—"}
                      </td>
                  ...
                  <tr>
                    <td colSpan={7} className="px-3 py-8 text-center md:px-6">
