...
function DiscountRequestDetailPage() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: () =>
      `Discount_Request_${request?.id || new Date().toISOString().split("T")[0]}`,
  });
  ...
  const handleAdminApprove = async () => {
    ...
    if (discountVal > originalAmount) {
      addToast(
        "error",
        "Discount amount cannot exceed the total assessment amount",
      );
      return;
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast(
          "error",
          response.message || "Failed to approve discount request",
        );
      }
    } catch (err) {
      addToast(
        ...
        <div className="mx-auto max-w-7xl space-y-4 p-4 md:p-6">
          <div className="rounded-2xl bg-red-50 border border-red-200 p-8 text-center">
            ...
            <p className="text-slate-600 mb-5">
              The discount or waiver request you are looking for does not exist
              or has been removed.
            </p>
            <Link
              href="/it/discounts"
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="flex items-center gap-2.5">
                <button
                  onClick={() => router.push("/it/discounts")}
              ...
              <p className="mt-1 text-xs md:text-sm text-slate-500 ml-11">
                Submitted on {formatDateTime(request.createdAt)} • Ref:{" "}
                {request.payment?.reference || "N/A"}
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {request.approver && (
                  <div>
                    ...
                    <p className="font-medium text-slate-800">
                      {request.approver.adminName || request.approver.email} (
                      {request.approver.center || "HQ"})
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="rounded-2xl bg-white p-5 md:p-6 border border-slate-200 shadow-xs flex flex-col justify-between">
              ...
              <div className="mt-4 space-y-3 text-sm">
                ...
                <div className="space-y-1.5 pt-1 text-xs text-slate-600">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Center: {request.member?.center || "General Center"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100">
              <Link
                href={`/it/entities/${request.member?.uid || request.memberId}`}
          ...
          <div className="rounded-2xl bg-white p-5 md:p-6 border border-slate-200 shadow-xs flex flex-col justify-between">
                ...
                <div className="rounded-xl bg-slate-50 p-3.5 border border-slate-100 space-y-2 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-slate-700">
                      {formatCurrency(request.payment?.paid || 0)}
                    </span>
                  </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100">
              <Link
                href={`/it/payments/${request.payment?.id || request.paymentId}`}
                className="text-xs font-bold text-emerald-600 hover:text-emerald-700 transition-colors inline-flex items-center gap-1"
              >
          ...
          <div className="flex items-center gap-2 border-b border-slate-100 pb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Grant discount approval or reject the waiver request with an
                official note.
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-slate-200 pb-4 mb-4">
              <div>
                ...
                <div className="text-slate-500 text-[11px] mt-1">
                  Abuja Municipal Area Council, FCT Abuja
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border border-slate-200 rounded-lg p-3">
                ...
                <div>
                  <strong>
                    {request.member?.businessName || request.member?.fullname}
                  </strong>
                  <br />
            ...
            <div className="grid grid-cols-2 gap-8 pt-6 border-t border-slate-200 mt-8">
              ...
              <div>
                <div className="border-b border-slate-400 pb-8 mb-1" />
                <div className="font-bold text-slate-900">
                  Revenue Authorizing Officer
                </div>
                <div className="text-[10px] text-slate-500">
                  Audit & Waiver Approval Division
                </div>
              </div>
            </div>
          </div>
