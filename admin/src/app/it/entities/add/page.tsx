...
export default function AddEntityPage() {
  ...
  const [metaData, setMetaData] = useState<{ page: number; limit: number }>({
    page: 1,
    limit: 100,
  });
  ...
  // Load all centers (admins represent centers)
  const fetchCenters = useCallback(async () => {
    try {
      ...
      addToast("error", e.message || "Failed to load centers");
    }
  }, [addToast]);
  ...
  const fetchPricing = useCallback(async () => {
    ...
    try {
      const pricingRes = await getPricingByCenter(selectedCenter);
    ...
    try {
      ...
      const matchingPricing = pricingOptions.filter((p) => {
        const cats = Array.isArray(p.category)
          ...
          : [p.category || ""];
        return cats.some(
          (cat) => String(cat || "").toUpperCase() === normalizedCategory,
        );
      });
      ...
      setCompanies(
        companyList.filter((c) => {
          ...
          return cats.some((cat) =>
            matchingPricing.map((p) => p.id).includes(String(cat || "")),
          );
        }),
      );
  ...
  useEffect(() => {
    if (locationStateValue) {
      const lgas =
        (statesData as Record<string, string[]>)[locationStateValue] || [];
  ...
  const validateForm = (): boolean => {
    ...
    if (!selectedCenter) {
      addToast("error", "Please select a center");
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res?.ok) {
        throw new Error(
          res?.message || res?.error || "Failed to create entity",
        );
      }

      addToast("success", "Entity created successfully");
      router.push("/it/entities");
    } catch (e) {
  ...
  useEffect(() => {
    if (formData.type && formData.category && selectedCenter) {
      fetchCompany();
      return;
    }
    setCompanies([]);
    setSelectedCompany(null);
  }, [fetchCompany, formData.category, formData.type, selectedCenter]);
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleLocationFieldChange("lga", e.target.value)
                    }
                    className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-700 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 appearance-none"
                  >
                    <option value="" hidden>
                ...
                <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-emerald-700">
                        {selectedLocation.nearestBusStop}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {priceLoading ? (
              ...
              <div className="space-y-4 pb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {getFilteredPricing().map((pricing) => {
                    const isSelected = (formData.pricing || []).includes(
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <p
                              className={`font-semibold ${isSelected ? "text-emerald-700" : "text-slate-900"}`}
                            >
                              {pricing.title}
                            </p>
                          </div>
                          ...
                          <p
                            className={`mt-1 text-xs font-bold ${isSelected ? "text-emerald-700" : "text-slate-600"}`}
                          >
                          ...
                          {pricing.code && (
                            <p
                              className={`mt-1 text-xs ${isSelected ? "text-emerald-700" : "text-slate-600"}`}
                            >
                              {pricing.code}
                            </p>
                          )}
              ...
              <div className="py-8 text-center">
                <p className="text-sm text-slate-500">
                  No pricing plans available for this center.
                </p>
              </div>
            )}
          </div>
