...
import { getCompanies, getAllCompanies } from "@/lib/services/company";
import { dashboardStats } from "@/lib/services/admin";

function Home() {
  const { user, role } = useAuth();
  // IT users see system-wide stats ("ADMIN" = all centers on the API)
  const userId = role === "IT" ? "ADMIN" : user?.uid || "ADMIN";
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const memberData = await getAllMembers(1, 1000);
        ...
        // fetch companies (all companies for IT, center-scoped otherwise)
