...
export default function ITAddPropertyPage() {
  ...
  // Load all available revenue centers for the IT property center selection dropdown
  useEffect(() => {
    let isMounted = true;
    const loadCenters = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          setFormData((prev) => (!prev.center && activeList.length > 0 ? { ...prev, center: activeList[0].uid } : prev));
        }
      } catch (err) {
        try {
          ...
          if (isMounted) {
            setCenters(pubList);
            setFormData((prev) => (!prev.center && pubList.length > 0 ? { ...prev, center: pubList[0].uid } : prev));
          }
        } catch (pubErr) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.center.trim()) {
      addToast("error", "Please select an administrative revenue center.");
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-8">
      ...
      <div>
        ...
        >
          <ArrowLeft size={16} />
          Back to IT Property Registry
        </Link>
      </div>

      {/* Header Banner */}
      <div className="rounded-2xl bg-gradient-to-r from-emerald-50 via-white to-teal-50 p-6 ring-1 ring-emerald-100 shadow-xs">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-800">Register New Property (IT Portal)</h1>
            <p className="text-sm text-slate-600">
              Create physical premises, commercial plazas, residential structures, or market stalls and assign them to any revenue center.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
              >
