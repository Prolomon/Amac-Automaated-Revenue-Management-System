...
function RecordDetailPage() {
    ...
    const curDoc = useRef<HTMLDivElement>(null);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            addToast("error", fetchError?.message || "Failed to load payment record");
        } finally {
            setLoading(false);
        }
    }, [recordId, addToast]);
    ...
    if (!record) {
        return (
            <div className="mx-auto max-w-7xl space-y-4 p-4 md:p-6">
                <button
                    type="button"
                    onClick={() => router.push(`/it/partners/${partnerId}/finance`)}
    ...
    const handleDownloadReceipt = () => {
        const element = curDoc.current;
        const options = {
            margin: 1,
            filename: 'custom-document.pdf',
            image: { quality: 0.98 },
            html2canvas: { scale: 2 }, // Higher scale means better crispness/quality
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
                        ...
                        <div className="grid gap-3">
                            <button
                                type="button"
                                className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-700 shadow-sm transition-colors hover:bg-emerald-50"
                            >
