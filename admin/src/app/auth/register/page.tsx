...
import type { Admin, AdminLocation } from "@/lib/services/admin";
...
export default function RegisterPage() {
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			addToast("success", "Admin account created successfully. Redirecting to login...");
	...
	const handleLocationChange = async (value: string) => {
		...
		try {
			...
			addToast("error", "Failed to fetch location suggestions");
			setSuggestions([]);
		}
	};
					...
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						...
						<div>
							...
							<input
								...
								className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-slate-900 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-200 appearance-none uppercase"
								placeholder="MAIN"
							/>
						</div>
