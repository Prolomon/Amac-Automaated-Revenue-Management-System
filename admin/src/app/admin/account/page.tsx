...
import { Staff } from "@/lib/services/staff";
...
export default function AccountPage() {
        ...
        try {
            if (user) {
                if (isAdmin) {
                    ...
                    // For staff, map to a minimal form shape for display
                    const staffUser = user as Staff;
                    setForm({
                        id: staffUser?.id ? String(staffUser.id) : "",
                        uid: staffUser?.uid ? String(staffUser.uid) : "",
                        center: staffUser?.center ?? "",
                        email: staffUser?.email ?? "",
                        phone: staffUser?.phone ?? "",
                        adminName: staffUser?.fullname ?? "",
                        adminEmail: "",
                        adminPhone: "",
                        adminLocation: staffUser?.location ?? "",
                        state: "",
                        address: "",
                        lga: "",
                        country: "",
                        location: { latitude: 0, longitude: 0 },
                        status: staffUser?.status !== false,
                    });
                }
            } else {
                setForm(null);
    ...
    const handleEditOrUpdate = async () => {
        ...
        try {
            ...
            addToast("success", "Account details updated successfully");
        } catch (error) {
            addToast("error", "Failed to update account details");
        } finally {
            setSaving(false);
            setEditing(false);
        }
    ...
    const handleUseCurrentLocation = () => {
        if (typeof navigator === "undefined" || !navigator.geolocation) {
            addToast("error", "Geolocation is not supported on this browser.");
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                addToast("success", "Location updated from your current device position");
                setLocating(false);
            },
            (geoError) => {
                ...
                <div className={`grid grid-cols-1 gap-3 ${editing ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}>
                    {isAdmin && (
                        <button
                            ...
                            disabled={saving}
                            className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-colors disabled:cursor-not-allowed ${editing
                                ? "bg-amber-500 hover:bg-amber-600 text-neutral-950 disabled:bg-amber-300"
