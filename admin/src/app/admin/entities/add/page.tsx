...
const STEPS = [
  { id: 1, title: "Classification", desc: "Type, category & zone" },
...
export default function AdminAddEntityPage() {
  ...
  const adminState = user?.state || "Abuja";
  const adminLga = user?.lga || "Abuja Municipal Area Council";
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      const companiesRes = await getCompanies(centerId, 1, 100);
  ...
  }, [centerId, formData.category, pricingOptions]);

  useEffect(() => {
    if (centerId) {
      fetchCompanies();
    }
  }, [centerId, fetchCompanies]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = {
        ...
        center: centerId || formData.center,
      ...
      <div className="space-y-6 p-4 md:p-6">
        <div className="rounded-2xl border border-emerald-200 bg-white p-6 md:p-10 shadow-sm text-center max-w-3xl mx-auto">
          ...
          <span className="mt-6 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3.5 py-1 text-xs font-semibold text-emerald-800 border border-emerald-200">
            <Sparkles size={14} /> Entity Created Successfully
          </span>
          ...
          <p className="mt-2 text-sm text-slate-600 max-w-lg mx-auto">
            The new member entity and premises have been registered in the council database.
          </p>
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div className="rounded-2xl bg-linear-to-r from-emerald-50 via-white to-cyan-50 p-5 md:p-6 ring-1 ring-emerald-100">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Create and register a new taxpayer, statutory document, and property premises in the municipal revenue system
        </p>
      </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Set the entity type, economic category, and municipal valuation rating zone.
                </p>
              </div>

              <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-600 text-white shrink-0">
                    <ShieldCheck size={20} />
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase font-semibold">Active Revenue Center</p>
                    <p className="text-sm font-bold text-slate-900">
                      {user?.adminName || user?.center || centerId || "AMAC Secretariat"}
                    </p>
                    <p className="text-xs text-slate-600">
                      {adminLga}, {adminState} &bull; Center ID: {centerId}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                      For registered commercial establishments, shops, hotels, banks, markets, and corporations.
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                      For private landlords, residential tenants, sole traders, and individual citizens.
                    </p>
                  </div>
                </div>
                {errors.type && (
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Provide the contact person, registered business identity, and assessment schedule.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className={formData.type === "INDIVIDUAL" ? "md:col-span-2" : ""}>
                  <label className="block text-sm font-semibold text-slate-800 mb-2">
                    {formData.type === "BUSINESS" ? "Contact Person / Representative Name *" : "Full Legal Name *"}
                  </label>
                  <div className="relative">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g., Suite 2B Royal Plaza"
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Assign a collection partner company (if applicable) and record the property address.
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-semibold text-slate-500">
                    {selectedCompany ? `Assigned: ${selectedCompany.name}` : "Direct Municipal Council"}
                  </span>
                </div>
                ...
                {companyLoading ? (
                  ...
                  <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-3.5 text-xs text-slate-600">
                    No partner company associated with this category. The entity will be directly registered under council management.
                  </div>
                )}
              </div>
          ...
          {currentStep === 6 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Assign initial revenue heads / pricing plans and verify complete entity registration data before submitting.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                {priceLoading ? (
                  ...
                  <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center text-xs text-slate-500">
                    No active tariffs match this category. Tariffs can also be assigned later after registration.
                  </div>
                )}
              </div>
