...
function PartnerFinancePage() {
    ...
    }, [addToast, partnerId, fromDate, toDate, query]);
    ...
    const handleDownloadStatement = () => {
        ...
        addToast("success", "Download statement functionality not yet implemented");
    };
    ...
    return (
        <div className="mx-auto max-w-7xl p-4 md:p-6 space-y-4 md:space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-100 shadow-sm">
                    <p className="text-xs uppercase tracking-wide text-slate-500">Settled Amount</p>
                    <p className="mt-2 text-2xl font-bold text-emerald-600">{formatCurrency(balances.totalRevenue)}</p>
                    <p className="text-xs text-slate-500 mt-1">Already settled</p>
                </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 transition-colors"
                        >
                                    ...
                                    records.map((record) => (
                                        <tr key={record.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50 transition-colors">
                                            <td className="py-4 text-sm font-medium text-slate-800">
                                                <Link href={`/admin/partners/${partnerId}/finance/${record.reference}`} className="hover:text-emerald-700">
                                                    {record.reference.slice(0, 27) + (record.reference.length > 27 ? "..." : "")}
                                                </Link>
                                            </td>
